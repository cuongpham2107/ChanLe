...
use App\Http\Controllers\dgaAdmin\MiniGame;
...
use App\Http\Controllers\dgaAdmin\ZalopayController;
...
class DUNGA extends Controller
{
    ...
    {
        $setting = Setting::first();
        $momo = Zalopay::where('status', '=', 1)->orderBy('show_order', 'desc')->get();
        if ($setting->theme == 'v1') {
        ...
        return response()->json(array(
            'status' => true,
            'message' => 'Thành công',
            'data_momo' => MomoResource::collection(Zalopay::where('status', 1)->orWhere('status', 0)->orderBy('show_order', 'desc')->get()),
            'game' => array(
    ...
    public function momoV2()
    {
        $setting = Setting::first();
        $momo = Momo::where('status', '=', 1)->orderBy('show_order', 'desc')->get();
        if ($setting->theme == 'v1') {
            $html = view('gameV1', compact('momo', 'setting'))->render();
        } else if ($setting->theme == 'v3') {
            $html = view('gameV3', compact('momo', 'setting'))->render();
    ...
    public function addHiz(Request $request)
    {
        $setting = Setting::first();
        $validator = Validator::make($request->all(), [
            'partnerId' => 'required|string|max:11',
            'partnerName' => 'required',
            'phoneSend' => 'required|string|max:11|exists:zalopays,phone',
            'comment' => 'required',
            'amount' => 'numeric|required',
            'receive' => 'numeric|required',
        ...
        $checkHis = HistoryPlay::where('tranId', $request->tranId)->first();

        if ($checkHis) {
            return response()->json(['status' => 'danger', 'message' => 'Đã tồn tại GD']);
        }
        $history = HistoryPlay::create($request->all());
        if ($setting->hu == 1) {
            (new \App\Http\Controllers\dgaAdmin\MiniGame)->xuLyHu($request->tranId, $request->partnerId);
        }

        if ($history) {
            return response()->json(['status' => 'success', 'message' => "Thành công", 'data' => $history]);
        }
        return response()->json(['status' => 'danger', 'message' => "Thất bại"]);
    }
    ...
    public function checkDayMission(Request $request)
    {
        ...
        if ($amount <= 0) {
            ...
            for ($i = $turn; $i < count($receiveLevel); $i++) {
                ...
                if ($token) {
                    $url = route('admin.checkMomo', ['phone' => $token->phone, 'receiver' => $request->phone]);
    ...
    {
        $setting = Setting::first();
        $momo = Momo::pluck('phone')->toArray();
        // TUẦN NÀY SỐ NGƯỜI CHƠI
        $top5 = DB::table('history_plays')
            ->select('partnerId as phone')
            ->selectRaw('sum(amount) as amount')
            ->whereNotIn('partnerId', $momo)
        ...
        $dga = 0;
        $key = 1;
        if ($setting->top_phone != null) { //fake top
            $top = explode('|', $setting->top_phone);
            $amount = explode('|', $setting->top_amount);
            for ($i = 0; $i < 5; $i++) {
                $results[$i]['key'] = $key++;
                $results[$i]['phone'] = substr($top[$i], 0, -8) . '****';
                $results[$i]['amount'] = $amount[$i];
                $results[$i]['gift'] = $gift[$i];
            }
        } else { //no fake top
            foreach ($top5 as $row) {
                if ($dga < count($gift)) {
                    if ($row->amount > 0) {
                        $results[$dga]['key'] = $key++;
                        $results[$dga]['phone'] = substr($row->phone, 0, -8) . '****';
                        $results[$dga]['amount'] = $row->amount;
                        $results[$dga]['gift'] = $gift[$dga];
                        $dga++;
                    }
                }
            }
    ...
    public function oldWeekTop()
    {
        ...
        foreach ($ListSDT as $row) {
            ...
            $ListUser[$dga]['amount'] = HistoryPlay::whereBetween('created_at', [Carbon::now()->startOfWeek(), Carbon::now()->endOfWeek()])->where(['partnerId' => $row])->sum('amount');
            $dga++;
        }
        $UserTop = [];
        $st = 0;
    ...
    public function checkTran(Request $request)
    {
        ...
        if (empty($data)) {
            ...
            if ($setting->refund == 1) {
                if ($data->status == 1 && $data->pay == 0) {
                    return response()->json(array('status' => true, 'data' => array('status' => true, 'message' => '<b> Chiến thắng</b> <br> <b> Mã giao dịch: #' . $data->tranId . ' </b> <br> <b> Tiền cược: ' . number_format($data->amount) . ' </b> <br> <b class="text-success"> Tiền nhận: ' . number_format($data->receive) . ' </b> <br> <b> Nội dung: ' . $data->comment . ' </b> <br> <b> Thời gian: ' . $data->created_at . ' </b> <hr> <b> Trạng thái: <span class="text-warning">Chờ xử lý</span> </b>')));
                    ...
                    return response()->json(array('status' => true, 'data' => array('status' => true, 'message' => '<b>Chiến thắng</b> <br> <b> Mã giao dịch: #' . $data->tranId . ' </b> <br> <b> Tiền cược: ' . number_format($data->amount) . ' </b> <br> <b class="text-success"> Tiền nhận: ' . number_format($data->receive) . ' </b> <br> <b> Nội dung: ' . $data->comment . ' </b> <br> <b> Thời gian: ' . $data->created_at . ' </b> <hr> <b> Trạng thái: <span class="text-warning">Chuyển lỗi, liên hệ ADMIN</span> </b>')));
                } else if ($data->status == 1 && $data->pay == 5) {
                    return response()->json(array('status' => true, 'data' => array('status' => true, 'message' => '<b>Chiến thắng</b> <br> <b> Mã giao dịch: #' . $data->tranId . ' </b> <br> <b> Tiền cược: ' . number_format($data->amount) . ' </b> <br> <b class="text-success"> Tiền nhận: ' . number_format($data->receive) . ' </b> <br> <b> Nội dung: ' . $data->comment . ' </b> <br> <b> Thời gian: ' . $data->created_at . ' </b> <hr> <b> Trạng thái: <span class="text-info">Chờ thanh toán</span> </b>')));
                } else if ($data->status == 2) {
                    return response()->json(array('status' => true, 'data' => array('status' => false, 'message' => '<b>Thua cuộc</b> <br> <b> Mã giao dịch: #' . $data->tranId . ' </b> <br> <b> Tiền cược: ' . number_format($data->amount) . ' </b> <br> <b class="text-success"> Tiền nhận: ' . number_format($data->receive) . ' </b> <br> <b> Nội dung: ' . $data->comment . ' </b> <br> <b> Thời gian: ' . $data->created_at . ' </b> <hr> <b> Trạng thái: <span class="text-danger">Sai nội dung</span> </b> <br> <button type="submit" class="btn btn-danger" style="margin-top: 10px;" onclick="DUNGA.refund()" >Hoàn tiền</button>')));
                } else if ($data->status == 3) {
                    return response()->json(array('status' => true, 'data' => array('status' => false, 'message' => '<b>Thua cuộc</b> <br> <b> Mã giao dịch: #' . $data->tranId . ' </b> <br> <b> Tiền cược: ' . number_format($data->amount) . ' </b> <br> <b class="text-success"> Tiền nhận: ' . number_format($data->receive) . ' </b> <br> <b> Nội dung: ' . $data->comment . ' </b> <br> <b> Thời gian: ' . $data->created_at . ' </b> <hr> <b> <b> Trạng thái: <span class="text-danger">Sai mức cược</span> </b> <br> <button type="submit" class="btn btn-danger" style="margin-top: 10px;" onclick="DUNGA.refund()" >Hoàn tiền</button>')));
                } else if ($data->status == 6) {
                    return response()->json(array('status' => true, 'data' => array('status' => false, 'message' => '<b>Thua cuộc</b> <br> <b> Mã giao dịch: #' . $data->tranId . ' </b> <br> <b> Tiền cược: ' . number_format($data->amount) . ' </b> <br> <b class="text-success"> Tiền nhận: ' . number_format($data->receive) . ' </b> <br> <b> Nội dung: ' . $data->comment . ' </b> <br> <b> Thời gian: ' . $data->created_at . ' </b> <hr> <b> <b> Trạng thái: <span class="text-danger">Chờ hoàn tiền</span> </b> <br> <button type="submit" class="btn btn-danger" style="margin-top: 10px;" onclick="DUNGA.refund()" >Hoàn tiền</button>')));
                } else if ($data->status == 4) {
                    return response()->json(array('status' => true, 'data' => array('status' => false, 'message' => '<b>Thua cuộc</b> <br> <b> Mã giao dịch: #' . $data->tranId . ' </b> <br> <b> Tiền cược: ' . number_format($data->amount) . ' </b> <br> <b class="text-success"> Tiền nhận: ' . number_format($data->receive) . ' </b> <br> <b> Nội dung: ' . $data->comment . ' </b> <br> <b> Thời gian: ' . $data->created_at . ' </b> <hr> <b> <b> Trạng thái: <span class="text-danger">Đã hoàn tiền</span> </b>')));
                } else if ($data->status == 5) {
                    return response()->json(array('status' => true, 'data' => array('status' => false, 'message' => '<b>Chờ kết quả</b> <br> <b> Mã giao dịch: #' . $data->tranId . ' </b> <br> <b> Tiền cược: ' . number_format($data->amount) . ' </b>  <b> Nội dung: ' . $data->comment . ' </b> <br> <b> Thời gian: ' . $data->created_at . ' </b> <hr> <b> ')));
                }
            } else {
                if ($data->status == 0 && $data->comment != NULL) {
                    return response()->json(array('status' => true, 'data' => array('status' => false, 'message' => '<b> Thua cuộc </b> <br> <b> Mã giao dịch: #' . $data->tranId . ' </b> <br> <b> Tiền cược: ' . number_format($data->amount) . ' </b> <br> <b class="text-success"> Tiền nhận: ' . number_format($data->receive) . ' </b> <br> <b> Nội dung: ' . $data->comment . ' </b> <br> <b> Thời gian: ' . $data->created_at . ' </b> ')));
                    ...
                    return response()->json(array('status' => true, 'data' => array('status' => false, 'message' => '<b>Thua cuộc</b> <br> <b> Mã giao dịch: #' . $data->tranId . ' </b> <br> <b> Tiền cược: ' . number_format($data->amount) . ' </b> <br> <b class="text-success"> Tiền nhận: ' . number_format($data->receive) . ' </b> <br> <b> Nội dung: ' . $data->comment . ' </b> <br> <b> Thời gian: ' . $data->created_at . ' </b> <hr> <b> Trạng thái: <span class="text-danger">Sai nội dung</span> </b>')));
                } else if ($data->status == 3) {
                    return response()->json(array('status' => true, 'data' => array('status' => false, 'message' => '<b>Thua cuộc</b> <br> <b> Mã giao dịch: #' . $data->tranId . ' </b> <br> <b> Tiền cược: ' . number_format($data->amount) . ' </b> <br> <b class="text-success"> Tiền nhận: ' . number_format($data->receive) . ' </b> <br> <b> Nội dung: ' . $data->comment . ' </b> <br> <b> Thời gian: ' . $data->created_at . ' </b> <hr> <b> <b> Trạng thái: <span class="text-danger">Sai mức cược</span> </b> ')));
                } else if ($data->status == 4) {
                    return response()->json(array('status' => true, 'data' => array('status' => false, 'message' => '<b>Thua cuộc</b> <br> <b> Mã giao dịch: #' . $data->tranId . ' </b> <br> <b> Tiền cược: ' . number_format($data->amount) . ' </b> <br> <b class="text-success"> Tiền nhận: ' . number_format($data->receive) . ' </b> <br> <b> Nội dung: ' . $data->comment . ' </b> <br> <b> Thời gian: ' . $data->created_at . ' </b> <hr> <b> <b> Trạng thái: <span class="text-danger">Đã hoàn tiền</span> </b>')));
    ...
    public function refund(Request $request)
    {
        ...
        if ($setting->refund == 1) {
            $tranId = $request->tran_id;
            $data = HistoryPlay::where('tranId', $tranId)->whereIn('status', [2, 3, 6])->first();
            if (empty($data)) {
                ...
                if ($limit < $setting->limit_refund) {
                    $momoPayment = (new MiniGame())->getMomoPayment($data);
                    if ($momoPayment) {
                        $amount = $data->amount * $setting->ratioHoan;
                        $dga_func_momo = new MomoController();
                        $data->status = 4;
                        $data->receive = $amount;
                        $data->save();
                        $token = $momoPayment->token;
                        $cmt = $setting->content_refund . ' ' . Str::upper(Str::random(6));
                        $res = $dga_func_momo->sendMoneyMomo($token, $amount, $cmt, $data->partnerId);
    ...
    public function checkGiftcode(Request $request)
    {
        ...
        $amountPlayToday = HistoryPlay::whereDate('created_at', Carbon::today()->toDateString())->where('partnerId', $phone)->sum('amount');

        // if ($amountPlayToday < 10000) {
        //     return response()->json(array('status' => false, 'message' => 'Vui lòng chơi 10k để nhận thưởng nhé'));
        // }
        ...
        return DB::transaction(function () use ($code, $phone, $setting) {
            // You can also use `findOrFail(1)` or any other query builder functions here depending on your needs.
            $gift = GiftCode::lockForUpdate()->where(['code' => $code, 'status' => 1])->first();
            // $user->update([
            ...
            if ($gift) {
                ...
                if ($his) {
                    ...
                    $token = Zalopay::where('status', 1)->inRandomOrder()->first()->token;
                    if ($token) {

                        $cmt = $setting->content_giftcode . ' ' . Str::upper(Str::random(6));

                        $dga_func_momo = new ZalopayController();
