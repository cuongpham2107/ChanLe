        ...
        if (
            ...
            if (
                ...
                for (t in e.queryParams) {
                    if (
                        e.queryParams.hasOwnProperty(t) &&
                        void 0 !== e.queryParams[t]
                    )
                        if (
                            e.disableCSV &&
                            Array.isArray(e.queryParams[t]) &&
                            e.queryParams[t].length
                        )
                            for (var u = 0; u < e.queryParams[t].length; u++)
                                (s = e.queryParams[t][u]),
                                    i.push(t + "=" + l(s));
                        else
                            (s = n
                                ? e.queryParams[t].toLowerCase()
                                : e.queryParams[t]),
                                i.push(t + "=" + l(s));
                }
                a += "?" + i.join("&");
            ...
            e.hash &&
                (a += n
                    ? "#" + String(e.hash).trim().toLowerCase()
                    : "#" + String(e.hash).trim());
        }
        return a;
    }
...
let DUNGA = (function () {
        ...
        initAjax({
            ...
            success: function (data) {
                if (data.status == true) {
                    ...
                    if (data.data_momo) {
                        let status_momo = {
                            0: `<span class="label label-warning text-uppercase">Bảo trì</span>`,
                        ...
                        for (let momo of data.data_momo) {
                            ...
                            const send_time_today =
                                settings.transfers_today.times;
                            var status_label = status_momo[momo.status];
                            if (send_time_today >= 190) {
                                status_label = `<span class="label label-warning text-uppercase">Bảo trì</span>`;
                            } else if (send_time_today >= 180) {
                                status_label = `<span class="label label-info text-uppercase">Sắp full</span>`;
                            }
                            if (settings.transfers_today == undefined) {
                                settings.transfers_today = {
                                    times: 0,
                            ...
                            table_one += `<td>${status_label}</td>`;
                            table_one += `<td><strong><span class="text-danger">${settings.transfers_today.times}</span> lần</strong></td>`;
                            table_one += `<td><strong><span class="text-danger cash-format">${number_format(
                                settings.transfers_today.amount
                            )}</span> / 50.000.000 VND</strong></td>`;
                            table_one += "</tr>";

                            table_two += `<tr>`;
                            table_two += `<td>${momo.username} <span class="label label-success text-uppercase" onclick="DUNGA.coppy('${momo.username}')"><i class="fa fa-clipboard" aria-hidden="true"></i></span></td>`;
                            table_two += `<td>${status_label}</td>`;
                            table_two += `<td><strong><span class="text-danger">${settings.transfers_today.times}</span> lần</strong></td>`;
                            table_two += `<td><strong><span class="text-danger cash-format">${number_format(
                                settings.transfers_today.amount
                            )}</span></strong></td>`;
                            table_two += "</tr>";
                        }
                    ...
                    if (data.game.active) {
                        for (let game of data.game.active) {
                            $(`button[data-game="${game}"]`).removeClass(
                                "hidden"
                            );
                        }
                        ...
                        if (ag == "" || ag == undefined) {
                            if (data.game.active.includes("chanle")) {
                                ...
                                $(
                                    `button[data-game="${data.game.active[0]}"]`
                                ).click();
                            }
                        } else {
        ...
        initAjax({
            ...
            success: function (data) {
                if (data.status == true) {
                    ...
                    if (data.active == 1) {
                        $("#author").html(
                            `<a href="${data.href}" target="_blank">${data.author}</a>`
                        );
                        let html_contact = "";
                        ...
                        if (data.day_top !== undefined && data.day_top == 1) {
                            loadDayTop();
                            $(".day_top").removeClass("hidden");
                        }
                        if (
                            data.day_mission !== undefined &&
                            data.day_mission == 1
                        ) {
                            loadMinigame({
                                game: "day_mission",
                        ...
                        if (
                            data.refer_friend !== undefined &&
                            data.refer_friend.active == 1
                        ) {
                            loadMinigame({
                                game: "refer_friend",
                                settings_encode:
                                    data.refer_friend.settings_encode,
                            });
                        }
                ...
                if (data.status == true) {
                    if (data.history.status == true) {
                        ...
                        for (let history of data.history.data) {
                            ...
                            html_history += `<td>${
                                status_win[history.victory]
                            }</td>`;
                            html_history += "</tr>";
                            if (
                                history.amount >= 50000 &&
                                noti_wined[history.id] == undefined
                            ) {
                                noti_win[history.id] = {
                                    game: "",
                ...
                if (data.status == true) {
                    if (data.weekTop.status == true) {
                        ...
                        for (let top of data.weekTop.data) {
                            html_weektop += "<tr>";
                            html_weektop += `<td><img src="/themes-v3/images/${top.key}.png" width="35px"></strong></span></td>`;
                            html_weektop += `<td class="col-xs-2"><span class="label label-success">${top.phone}</span></td>`;
                            html_weektop += `<td class="col-xs-5 text-center"><span class="label label-danger">${number_format(
                                top.amount
                            )}</span></td>`;
                            html_weektop += `<td class="col-xs-5 text-center"><span class="label label-danger">${number_format(
                                top.gift
    ...
    loadDayTop = () => {
        initAjax({
            ...
            success: function (data) {
                if (data.status == true) {
                    if (data.dayTop.status == true) {
                        let html_daytop = "";
                        for (let top of data.dayTop.data) {
                            html_daytop += "<tr>";
                            html_daytop += `<td><img src="/themes-v3/images/${top.key}.png" width="35px"></strong></span></td>`;
                            html_daytop += `<td class="col-xs-2"><span class="label label-success">${top.phone}</span></td>`;
                            html_daytop += `<td class="col-xs-5 text-center"><span class="label label-danger">${number_format(
                                top.amount
                            )}</span></td>`;
                            html_daytop += `<td class="col-xs-5 text-center"><span class="label label-danger">${number_format(
                                top.gift
                        ...
                        $(".day_top #day_top").html(html_daytop);
                        // console.log();
                    }
                }
            },
        ...
        initAjax({
            ...
            success: function (data) {
                if (data.status == "error") {
                    alert(
                        "Oh !! Số điện thoại này chưa chơi game nào, hãy kiểm tra lại"
                    );
                    $("#non_query").show();
                    $("#gift").hide();
                } else {
                ...
                if (data.status == true) {
                    if (data.data.status == true) {
                        ...
                        $(".ntmsg").html(`
                            ...
                            <div class="st"><b class="text-danger">Sá»‘ tiá»n: ${number_format(
                                data_response.amount
                            )} vnÄ‘</b></div>
                            <div class="st"><b class="text-warning">Lá»i nháº¯n: ${
                                data_response.comment
                            }</b></div>`);
                    }
                } else {
    ...
    return {
        ...
        joinDiemdanh: function () {
            ...
            let person = prompt(
                "Xác minh bạn là học sinh giỏi toán " +
                    num1 +
                    "+" +
                    num2 +
                    "= ?:",
        ...
        noti_win: function (data) {
            const { game, partnerId, amount } = data;
            new Notify({
                status: "success",
                title: "Trò chơi: " + game,
                text: `Chúc mừng <b>${partnerId}</b> đã thắng ${number_format(
                    amount
                )}`,
                autoclose: true,
                customIcon:
                    '<img src="https://i.imgur.com/D69n2Ak.png" style="width: 30px; height: 30px;">',
            });
        },
    };
})();
"undefined" != typeof module &&
    ...
    $(document).ready(function () {
        ...
        var pusher = new Pusher(pusher_key, {
            ...
            crypt_master_key_base64:
                DUNGASettings.pusher.crypt_master_key_base64,
        });
    });
            ...
            success: function (data_xhr) {
                if (data_xhr.status == true) {
                    Swal.fire({
                        title:
                            "Báº¡n Ä‘ang cĂ³ " +
                            DUNGA.number_format(data_xhr.data.amount) +
                            " vnÄ‘. Nháº­p sá»‘ tiá»n muá»‘n rĂºt:",
                        input: "number",
                        inputAttributes: {
                            autocapitalize: "off",
                        },
...
$("#result_hu [name=partnerId]").on("input", function () {
    if (this.value.length >= 10 && this.value.length <= 11) {
        $("#msg_hu2").html(
            '<button type="text" class="btn btn-success submit-join-hu" onclick="DUNGA.joinhu()">Tham gia</button>'
        );
    } else {
